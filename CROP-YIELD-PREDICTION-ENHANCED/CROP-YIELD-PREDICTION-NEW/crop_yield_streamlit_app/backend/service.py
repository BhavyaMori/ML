...
DATA_PATH = ROOT_DIR / "Crop_Yield_final.csv"
ARTIFACT_DIR = ROOT_DIR / "artifacts"
MODEL_PATH = ARTIFACT_DIR / "crop_yield_pipeline.joblib"
METADATA_PATH = ARTIFACT_DIR / "metadata.json"
NOTEBOOK_PATH = ROOT_DIR / "FINAL_CODE.ipynb"

DISPLAY_COLUMNS = ["fertilizer", "temp", "n", "p", "k", "yield"]
FEATURE_COLUMNS = ["fertilizer", "temp", "n", "p", "k"]
...
def _prepare_training_dataframe(raw_df: pd.DataFrame) -> pd.DataFrame:
    ...
    normalized = normalized.fillna(normalized.median(numeric_only=True))
    ...
    filtered = filtered[[c for c in DISPLAY_COLUMNS if c in filtered.columns]]
...
def predict_yield(fertilizer: float, temp: float, n: float, p: float, k: float) -> float:
    runtime = get_runtime()
    input_df = pd.DataFrame(
        [{"fertilizer": fertilizer, "temp": temp, "n": n, "p": p, "k": k}]
    )
...
def _feature_relationship_chart(processed_df: pd.DataFrame):
    corr = (
        processed_df[["fertilizer", "temp", "n", "p", "k", "yield"]]
